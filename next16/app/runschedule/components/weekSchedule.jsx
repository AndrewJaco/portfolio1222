...
export default function WeekSchedule({ allWeeks, days, thisWeekRow, upcomingWeeks, totalWeekNumber }) {
  ...
  const handleCapture = async (weekNumber) => {
    ...
    link.href = dataURL;
    link.download = `week-${weekNumber}-schedule.png`;
    link.click();
  };

  const visibleWeeks = showAllWeeks ? allWeeks : upcomingWeeks;
    ...
    <div className="w-full max-w-4xl space-y-6">
      ...
      {visibleWeeks.map((week, weekIndex) => (
        <div key={weekIndex} className="relative">
          <button
            onClick={() => handleCapture(week.weekNumber)}
            className="absolute right-4 top-4 w-7 h-7 flex items-center justify-center text-light rounded-full bg-primary-400 hover:bg-primary-400/80 cursor-pointer z-10"
          >
